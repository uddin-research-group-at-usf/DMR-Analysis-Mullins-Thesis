## DMR Analysis with mCSEA


########################################## set working directory and load the needed packages

## set workig directory
setwd('/Users/laniemullins/Desktop/School/MSPH_Genomics/Uddin_Lab /DNHS_Depression/Analysis')
...
######################################## prepare/format data never vs current
...
beta_vals_1 <- fread('beta_1.csv')
head(beta_vals_1)
beta_vals_1 <- column_to_rownames(beta_vals_1, var = 'V2')
column_to_delete <- 1
...
dim(beta_1_top)
## 40989 beta values and 63 samples, as expected

## remove last column -- the variance from the DF
delete_var_column <- 63
beta_1_top <- beta_1_top[, -delete_var_column, drop = FALSE]
head(beta_1_top)

beta_vals_2 <- fread('beta_2.csv')
head(beta_vals_2)
beta_vals_2 <- column_to_rownames(beta_vals_2, var = 'V2')
column_to_delete <- 1
beta_vals_2 <- beta_vals_2[, -column_to_delete, drop = FALSE]
...
# make sure all of the data is numeric, it not remove the column
# for some reason, there is an inserted column that is the probe names
data_types <- sapply(beta_vals_2, class)
print(data_types)
column_to_delete <- "V44"
beta_vals_2 <- beta_vals_2[, !(colnames(beta_vals_2) %in% column_to_delete), drop = FALSE]
...
beta_vals_3 <- fread('beta_3.csv')
head(beta_vals_3)
beta_vals_3 <- column_to_rownames(beta_vals_3, var = 'V2')
column_to_delete <- 1
beta_vals_3 <- beta_vals_3[, -column_to_delete, drop = FALSE]
...
print(data_types)
column_to_delete <- "V44"
beta_vals_3 <- beta_vals_3[, !(colnames(beta_vals_3) %in% column_to_delete), drop = FALSE]
...
cols_of_interest <- c("explainatory", "CD8T", "CD4T", "NK", "Bcell",
                      "Mono", "Neu", "female")
...
## Get only the information we need
#Binary MDD state, sex, cell type proportion
cols_of_interest_2 <- c("explainatory", "CD8T", "CD4T", "NK", "Bcell",
                        "Mono", "Neu", "female")


## prep pheno for current vs remitted
## load phenotype data
pheno_3 <- read.csv('pheno_3.csv', 
                    header = TRUE, 
                    sep = ",")
...
## Get only the information we need
#Binary MDD state, sex, cell type proportion
cols_of_interest_3 <- c("explainatory", "CD8T", "CD4T", "NK", "Bcell",
                        "Mono", "Neu", "female")


##################################### rank probes and run test
...
write.csv(myResults_sig_1, file = "never_current_significant_mcSEA_Results.csv", quote = FALSE)
...
## r rank probes and run test never vs remitted
...
write.csv(myResults_sig_2, file = "never_remitted_significant_mcSEA_Results.csv", quote = FALSE)
...
## r rank probes and run test current vs remitted
...
write.csv(myResults_sig_3, file = "current_remitted_significant_mcSEA_Results.csv", quote = FALSE)
...
doc <- doc %>% 
  ...
  print(target = "DNHS_mCSEA_Results_1.docx")
